...
import { fmt, today, getCat } from "../../utils/helpers";
import { Card, Badge, Inp, Sel, Btn } from "../ui";

const PER_PAGE = 8;

export default function Transactions({ expenses, categories, onDelete, onEdit, onAddNew }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 10, alignItems: "center" }}>
        <Inp placeholder="🔍  Search by description or merchant..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} style={{ flex: 1 }} />
      ...
      {showFilters && (
        <Card cls="fadein" style={{ marginBottom: 14, padding: "18px 22px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            ...
            {activeCount > 0 && <button onClick={clearAll} style={{ fontSize: 11, color: C.danger, background: "none", border: "none", cursor: "pointer", fontFamily: F }}>✕ Clear All</button>}
          </div>
          ...
          {activeCount > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 12 }}>
              {search    && <Badge color={C.accent2}>Search: "{search}" <button onClick={() => setSearch("")}    style={{ background: "none", border: "none", color: "inherit", cursor: "pointer", marginLeft: 3 }}>×</button></Badge>}
              {catF !== "all" && <Badge color={C.accent}>{catF} <button onClick={() => setCatF("all")}          style={{ background: "none", border: "none", color: "inherit", cursor: "pointer", marginLeft: 3 }}>×</button></Badge>}
              {methodF !== "all" && <Badge color={C.purple}>{methodF} <button onClick={() => setMethodF("all")} style={{ background: "none", border: "none", color: "inherit", cursor: "pointer", marginLeft: 3 }}>×</button></Badge>}
              {dateFrom  && <Badge color={C.warn}>From: {dateFrom} <button onClick={() => setDateFrom("")}      style={{ background: "none", border: "none", color: "inherit", cursor: "pointer", marginLeft: 3 }}>×</button></Badge>}
              {dateTo    && <Badge color={C.warn}>To: {dateTo} <button onClick={() => setDateTo("")}            style={{ background: "none", border: "none", color: "inherit", cursor: "pointer", marginLeft: 3 }}>×</button></Badge>}
              {minAmt    && <Badge color={C.danger}>Min: ${minAmt} <button onClick={() => setMinAmt("")}        style={{ background: "none", border: "none", color: "inherit", cursor: "pointer", marginLeft: 3 }}>×</button></Badge>}
              {maxAmt    && <Badge color={C.danger}>Max: ${maxAmt} <button onClick={() => setMaxAmt("")}        style={{ background: "none", border: "none", color: "inherit", cursor: "pointer", marginLeft: 3 }}>×</button></Badge>}
            </div>
          )}
        </Card>
      )}
      ...
      <Card cls="fadeup" style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "13px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: 12, color: C.muted }}>
            ...
            <strong style={{ color: C.accent }}> ${fmt(totalF)}</strong>
          </div>
              ...
              {paged.map(e => {
                ...
                return (
                  <tr key={e.id} className="row-hover" style={{ borderBottom: `1px solid ${C.border}`, transition: "background .12s" }}>
                    ...
                    <td style={{ padding: "10px 14px", color: C.danger, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", fontVariantNumeric: "tabular-nums" }}>-${fmt(e.amount)}</td>
        ...
        {pages > 1 && (
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 6, padding: "14px", borderTop: `1px solid ${C.border}` }}>
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 7, padding: "5px 11px", color: page === 1 ? C.faint : C.muted, cursor: page === 1 ? "default" : "pointer", fontFamily: F, fontSize: 12 }}>←</button>
            {Array.from({ length: pages }, (_, i) => i + 1).map(p => (
              <button key={p} onClick={() => setPage(p)} style={{ width: 30, height: 30, borderRadius: 7, border: `1px solid ${p === page ? C.accent : C.border}`, background: p === page ? C.accent + "22" : "transparent", color: p === page ? C.accent : C.faint, fontFamily: F, fontSize: 12, cursor: "pointer" }}>{p}</button>
            ))}
            <button onClick={() => setPage(p => Math.min(pages, p + 1))} disabled={page === pages} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 7, padding: "5px 11px", color: page === pages ? C.faint : C.muted, cursor: page === pages ? "default" : "pointer", fontFamily: F, fontSize: 12 }}>→</button>
          </div>
        )}
      </Card>
    </div>
